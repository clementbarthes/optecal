...
double	interpolator::InterpolatedValue
(
...
{ /* begin InterpolatedValue */
    ...
    long    Width = Bcoeff->cols;
    long    Height = Bcoeff->rows;
    ...
    for (j = 0L; j <= SplineDegree; j++) {
        p = &(Bcoeff->at<float>(0,0)) + (ptrdiff_t)(yIndex[j] * Width);
        w = 0.0;
...
double	interpolator::InterpolatedDerivativeX
(
        cv::Mat* Bcoeff,	/* input B-spline array of coefficients */
        double	x,			/* x coordinate where to interpolate */
        double	y,			/* y coordinate where to interpolate */
...
{ /* begin InterpolatedValue */
    ...
    float	*p;
    double	xWeight[10], yWeight[10];
    double	interpolated, minusDiffInterp;
    double	w, w2, w4, t, t0, t1;
    long	xIndex[10], yIndex[10];
    long    Width = Bcoeff->cols;
    long    Height = Bcoeff->rows;
    long	Width2 = 2L * Width - 2L, Height2 = 2L * Height - 2L;
    long	i, j, k;
    /* Interpolate at x-1/2 and at x+1/2 to obtain the first derivative in x*/
    x-=1.5;
    for(int diffIter = 0; diffIter<2;++diffIter){
        x +=1.;

        i = (long)floor(x + 0.5) - (SplineDegree-1) / 2L;
        j = (long)floor(y + 0.5) - SplineDegree / 2L;
        for (k = 0L; k <= SplineDegree; k++) {
            xIndex[k] = i++;
            yIndex[k] = j++;
        ...
        for (j = 0L; j <= SplineDegree; j++) {
            p = &(Bcoeff->at<float>(0,0)) + (ptrdiff_t)(yIndex[j] * Width);
            w = 0.0;
            for (i = 0L; i <= SplineDegree-1; i++) {
                w += xWeight[i] * p[xIndex[i]];
            }
            interpolated += yWeight[j] * w;
        }
        if(diffIter == 0){
            minusDiffInterp = -interpolated;
        }
    }
    return(interpolated+minusDiffInterp);
} /* end InterpolatedValue */

double	interpolator::InterpolatedDerivativeY
(
        cv::Mat* Bcoeff,	/* input B-spline array of coefficients */
        double	x,			/* x coordinate where to interpolate */
        double	y,			/* y coordinate where to interpolate */
...
{ /* begin InterpolatedValue */
    ...
    for(int diffIter = 0; diffIter<2;++diffIter){
        y +=1.;
        i = (long)floor(x + 0.5) - SplineDegree / 2L;
        j = (long)floor(y + 0.5) - (SplineDegree-1) / 2L;
        for (k = 0L; k <= SplineDegree; k++) {
            xIndex[k] = i++;
            yIndex[k] = j++;
        }
